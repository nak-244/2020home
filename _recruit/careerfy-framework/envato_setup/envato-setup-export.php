...
$post_types = array('attachment', 'post', 'page', 'job', 'candidate', 'employer', 'package', 'faq', 'wpcf7_contact_form');
    ...
    foreach ($post_datas as $post_data) {
        ...
        if ($post_type == 'attachment') {
            ...
            if (is_file($file)) {
                ...
                if (function_exists('careerfy_demo_data_get_path') && is_dir(careerfy_demo_data_get_path('content/'))) {
                    if (!is_dir(careerfy_demo_data_get_path('content/images/'))) {
                        mkdir(careerfy_demo_data_get_path('content/images/'), 0755);
                    }
                    $img_dir = careerfy_demo_data_get_path('content/images/');
...
$theme_options = array();
foreach ($all_options as $name => $value) {
    if (stristr($name, 'careerfy_framework_options') !== false) {
        $arr_value = maybe_unserialize($value);
...
if (function_exists('careerfy_demo_data_get_path')) {
    $dir = careerfy_demo_data_get_path('content/');
}
