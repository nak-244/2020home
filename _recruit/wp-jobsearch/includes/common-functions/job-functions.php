...
function jobsearch_job_postin_canedmsgs_init($_view_script = false)
{
    ...
    <script type="text/javascript">
        ...
        foreach ($temps_posts as $temps_postid) {
            ...
            $the_desc = isset($post_obj->post_content) ? $post_obj->post_content : '';
            $the_desc = apply_filters('the_content', $the_desc);
                                            ...
                                            <ul>
                                                <li<?php echo (is_user_logged_in() ? ' style="display: none;"' : '') ?>>
                                                    ...
                                                    <div class="input-field">
                                                        <input type="text" name="send_message_uname" value="<?php echo($user_displayname) ?>"/>
                                                    </div>
                                                </li>
                                                <?php
                                                if (!is_user_logged_in()) {
...
add_action('jobsearch_job_feature_expire_cron', array($this, 'jobsearch_job_feature_expire_cron_callback'));
        ...
        if (!empty($attall_users)) {
            foreach ($attall_users as $attch_usritm) {
                ...
                <li><a href="javascript:void(0);" class="atchuser-itm-btn"
                       data-id="<?php echo($attch_usritm['ID']) ?>" data-name="<?php echo($attch_usritm['post_title']) ?>"><?php echo($attch_usritm['post_title']) ?></a>
                    <span></span></li>
                <?php
            }
        }
...
if (!function_exists('jobsearch_jobmeta_serchemps_throgh_popup')) {
    ...
    function jobsearch_jobmeta_serchemps_throgh_popup()
    {
        ...
        if ($total_pages > 1) {
            ?>
            <a href="javascript:void(0);" class="lodmore-users-btn" data-tpages="<?php echo($total_pages) ?>"  data-keyword="<?php echo($keyword) ?>" data-gtopage="2"><?php esc_html_e('Load More', 'wp-jobsearch') ?></a>
            <?php
        }
        ...
        if (!empty($attall_users)) {
            foreach ($attall_users as $attch_usritm) {
                ?>
                <li>
                    <a href="javascript:void(0);" class="atchuser-itm-btn" data-id="<?php echo($attch_usritm['ID']) ?>" data-name="<?php echo($attch_usritm['post_title']) ?>"><?php echo($attch_usritm['post_title']) ?></a>
                    <span></span>
                </li>
                <?php
            }
