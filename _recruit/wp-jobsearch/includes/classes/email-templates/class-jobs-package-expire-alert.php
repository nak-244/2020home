...
if (!class_exists('jobsearch_jobs_package_expire_template')) {

    class jobsearch_jobs_package_expire_template {
        ...
        public function __construct() {
            ...
            add_action('init', array($this, 'jobsearch_jobs_package_expire_template_init'), 1, 0);
            add_filter('jobsearch_jobs_package_expire_filter', array($this, 'jobsearch_jobs_package_expire_filter_callback'), 1, 4);
            add_filter('jobsearch_email_template_settings', array($this, 'template_settings_callback'), 12, 1);
            add_action('jobsearch_jobs_package_expire_email', array($this, 'jobsearch_jobs_package_expire_callback'), 10, 2);
        }

        public function jobsearch_jobs_package_expire_template_init() {
            $this->user = array();
            $this->order_id = '';
            
            $this->rand = rand(0, 99999);
            $this->group = 'job';
            $this->type = 'jobs_package_expire';
            $this->filter = 'jobs_package_expire';
            $this->email_template_db_id = 'jobs_package_expire';
            $this->switch_label = esc_html__('Jobs Package Expiry', 'wp-jobsearch');
            $this->default_subject = esc_html__('Jobs Package Expire', 'wp-jobsearch');
            $this->default_recipients = '';
            $default_content = esc_html__('Default content', 'wp-jobsearch');
            $default_content = apply_filters('jobsearch_jobs_package_expire_filter', $default_content, 'html', 'jobs-package-expire', 'email-templates');
            $this->default_content = $default_content;
            $this->email_template_prefix = 'jobs_package_expire';
        ...
        public function jobsearch_jobs_package_expire_callback($user, $order_id = '') {
            ...
            if (isset($template['switch']) && $template['switch'] == 1) {
                ...
                jobsearch_jobs_package_expire_template::$is_email_sent1 = $this->is_email_sent;
                return true;
            }
        }
        ...
        public function jobsearch_jobs_package_expire_filter_callback($html, $slug = '', $name = '', $ext_template = '') {
            ob_start();
            $html = '';
            $template = '';
            if ($ext_template != '') {
        ...
        public function template_settings_callback($email_template_options) {
            ...
            $email_template_options['jobs_package_expire']['rand'] = $this->rand;
            $email_template_options['jobs_package_expire']['email_template_prefix'] = $this->email_template_prefix;
            $email_template_options['jobs_package_expire']['email_template_group'] = $this->email_template_group;
            $email_template_options['jobs_package_expire']['default_var'] = $this->default_var;
        ...
        public function get_total_jobs() {
            ...
            $total_jobs = get_post_meta($trans_order_id, 'num_of_jobs', true);
        ...
        public function get_used_jobs() {
            ...
            $jobs_list = get_post_meta($trans_order_id, 'jobsearch_order_jobs_list', true);
        ...
        public function get_remaining_jobs() {
            ...
            $total_jobs = get_post_meta($trans_order_id, 'num_of_jobs', true);
            $used_jobs = jobsearch_pckg_order_used_jobs($trans_order_id);
    ...
    new jobsearch_jobs_package_expire_template();
}