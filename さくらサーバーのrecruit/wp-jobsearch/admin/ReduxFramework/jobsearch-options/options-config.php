...
$args = array(
    ...
    'display_name' => 'JobSearch Options',
    ...
    'menu_title' => 'JobSearch Options',
    // The text to appear on the page title.
    'page_title' => 'JobSearch Options',
...
if (class_exists('Redux') && method_exists($redux_class, 'set_args')) {
    ...
    $sec_array[] = array(
        ...
        'subtitle' => __('It will disable all sectors from the site.', 'wp-jobsearch'),
        'desc' => '',
        'options' => array(
    ...
    Redux::set_section($opt_name, apply_filters('jobsearch_api_settings_section', $section_settings));
    //

    $section_settings = apply_filters('jobsearch_login_settings_section', array());

    if (isset($section_settings['title'])) {
        Redux::set_section($opt_name, $section_settings);
    }
    ...
    if (!empty($aftr_empdash_sec)) {
        if (isset($aftr_empdash_sec[0]['title'])) {
            foreach ($aftr_empdash_sec as $aftr_empdash_secitm) {
                Redux::set_section($opt_name, $aftr_empdash_secitm);
            }
        } else {
            Redux::set_section($opt_name, $aftr_empdash_sec);
        }
    }
    ...
    if (!empty($aftr_empdetail_sec)) {
        Redux::set_section($opt_name, $aftr_empdetail_sec);
    }
    ...
    if (!empty($section_settings['fields'])) {
        ...
        if (!empty($after_aplyjobsett_sec)) {
            Redux::set_section($opt_name, $after_aplyjobsett_sec);
        }
    ...
    if (!empty($aftr_jobdetail_sec)) {
        Redux::set_section($opt_name, $aftr_jobdetail_sec);
    }
    ...
    if (!empty($resume_export_sec)) {
        Redux::set_section($opt_name, $resume_export_sec);
    }
}