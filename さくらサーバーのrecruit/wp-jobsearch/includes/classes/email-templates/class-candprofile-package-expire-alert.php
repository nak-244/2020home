...
if (!class_exists('jobsearch_candprofile_package_expire_template')) {

    class jobsearch_candprofile_package_expire_template {
        ...
        public function __construct() {
            ...
            add_action('init', array($this, 'jobsearch_candprofile_package_expire_template_init'), 1, 0);
            add_filter('jobsearch_candprofile_package_expire_filter', array($this, 'jobsearch_candprofile_package_expire_filter_callback'), 1, 4);
            add_filter('jobsearch_email_template_settings', array($this, 'template_settings_callback'), 12, 1);
            add_action('jobsearch_candprofile_package_expire_email', array($this, 'jobsearch_candprofile_package_expire_callback'), 10, 2);
        }

        public function jobsearch_candprofile_package_expire_template_init() {
            $this->user = array();
            $this->order_id = '';
            
            $this->rand = rand(0, 99999);
            $this->group = 'job';
            $this->type = 'candprofile_package_expire';
            $this->filter = 'candprofile_package_expire';
            $this->email_template_db_id = 'candprofile_package_expire';
            $this->switch_label = esc_html__('Candidate Profile Package Expiry', 'wp-jobsearch');
            $this->default_subject = esc_html__('Profile Package Expire', 'wp-jobsearch');
            $this->default_recipients = '';
            $default_content = esc_html__('Default content', 'wp-jobsearch');
            $default_content = apply_filters('jobsearch_candprofile_package_expire_filter', $default_content, 'html', 'candprofile-package-expire', 'email-templates');
            $this->default_content = $default_content;
            $this->email_template_prefix = 'candprofile_package_expire';
        ...
        public function jobsearch_candprofile_package_expire_callback($user, $order_id = '') {
            ...
            if (isset($template['switch']) && $template['switch'] == 1) {
                ...
                jobsearch_candprofile_package_expire_template::$is_email_sent1 = $this->is_email_sent;
                return true;
            }
        }
        ...
        public function jobsearch_candprofile_package_expire_filter_callback($html, $slug = '', $name = '', $ext_template = '') {
            ob_start();
            $html = '';
            $template = '';
            if ($ext_template != '') {
        ...
        public function template_settings_callback($email_template_options) {
            ...
            $email_template_options['candprofile_package_expire']['rand'] = $this->rand;
            $email_template_options['candprofile_package_expire']['email_template_prefix'] = $this->email_template_prefix;
            $email_template_options['candprofile_package_expire']['email_template_group'] = $this->email_template_group;
            $email_template_options['candprofile_package_expire']['default_var'] = $this->default_var;
    ...
    new jobsearch_candprofile_package_expire_template();
}