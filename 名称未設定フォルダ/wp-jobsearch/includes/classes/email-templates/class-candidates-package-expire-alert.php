...
if (!class_exists('jobsearch_candidates_package_expire_template')) {

    class jobsearch_candidates_package_expire_template {
        ...
        public function __construct() {
            ...
            add_action('init', array($this, 'jobsearch_candidates_package_expire_template_init'), 1, 0);
            add_filter('jobsearch_candidates_package_expire_filter', array($this, 'jobsearch_candidates_package_expire_filter_callback'), 1, 4);
            add_filter('jobsearch_email_template_settings', array($this, 'template_settings_callback'), 12, 1);
            add_action('jobsearch_candidates_package_expire_email', array($this, 'jobsearch_candidates_package_expire_callback'), 10, 2);
        }

        public function jobsearch_candidates_package_expire_template_init() {
            $this->user = array();
            $this->order_id = '';
            
            $this->rand = rand(0, 99999);
            $this->group = 'job';
            $this->type = 'candidates_package_expire';
            $this->filter = 'candidates_package_expire';
            $this->email_template_db_id = 'candidates_package_expire';
            $this->switch_label = esc_html__('Candidates Package Expiry', 'wp-jobsearch');
            $this->default_subject = esc_html__('Candidates Package Expire', 'wp-jobsearch');
            $this->default_recipients = '';
            $default_content = esc_html__('Default content', 'wp-jobsearch');
            $default_content = apply_filters('jobsearch_candidates_package_expire_filter', $default_content, 'html', 'candidates-package-expire', 'email-templates');
            $this->default_content = $default_content;
            $this->email_template_prefix = 'candidates_package_expire';
            ...
            $this->codes = array(
                ...
                array(
                    'var' => '{total_apps}',
                    'display_text' => esc_html__('Total Applications', 'wp-jobsearch'),
                    'function_callback' => array($this, 'get_total_apps'),
                ),
                array(
                    'var' => '{used_apps}',
                    'display_text' => esc_html__('Used Applications', 'wp-jobsearch'),
                    'function_callback' => array($this, 'get_used_apps'),
                ),
                array(
                    'var' => '{remaining_apps}',
                    'display_text' => esc_html__('Remaining Applications', 'wp-jobsearch'),
                    'function_callback' => array($this, 'get_remaining_apps'),
                ),
                array(
                    'var' => '{expiry_date}',
        ...
        public function jobsearch_candidates_package_expire_callback($user, $order_id = '') {
            ...
            if (isset($template['switch']) && $template['switch'] == 1) {
                ...
                jobsearch_candidates_package_expire_template::$is_email_sent1 = $this->is_email_sent;
                return true;
            }
        }
        ...
        public function jobsearch_candidates_package_expire_filter_callback($html, $slug = '', $name = '', $ext_template = '') {
            ob_start();
            $html = '';
            $template = '';
            if ($ext_template != '') {
        ...
        public function template_settings_callback($email_template_options) {
            ...
            $email_template_options['candidates_package_expire']['rand'] = $this->rand;
            $email_template_options['candidates_package_expire']['email_template_prefix'] = $this->email_template_prefix;
            $email_template_options['candidates_package_expire']['email_template_group'] = $this->email_template_group;
            $email_template_options['candidates_package_expire']['default_var'] = $this->default_var;
        ...
        public function get_total_apps() {

            $trans_order_id = $this->order_id;
            
            $total_apps = get_post_meta($trans_order_id, 'num_of_apps', true);
        ...
        public function get_used_apps() {

            $trans_order_id = $this->order_id;
            
            $apps_list = get_post_meta($trans_order_id, 'jobsearch_order_apps_list', true);

            if (!empty($apps_list)) {
                $apps_list_count = count(explode(',', $apps_list));
            }
            
            $apps_list_count = $apps_list_count > 0 ? $apps_list_count : 0;
            
            $apps_list_count = absint($apps_list_count);
            
            return $apps_list_count;
        }
        
        public function get_remaining_apps() {

            $trans_order_id = $this->order_id;
            
            $total_apps = get_post_meta($trans_order_id, 'num_of_apps', true);
            $used_apps = jobsearch_pckg_order_used_apps($trans_order_id);
    ...
    new jobsearch_candidates_package_expire_template();
}