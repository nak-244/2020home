...
if (!class_exists('Envato_Theme_Setup_Wizard')) {
    ...
    class Envato_Theme_Setup_Wizard {
        ...
        private function _get_plugins() {
            ...
            $plugins_list = $instance->plugins;
            if (isset($plugins_list['wp-jobsearch'])) {
                $wp_jobsearch_data = $plugins_list['wp-jobsearch'];
                $jobsearch_arr = array('wp-jobsearch' => $wp_jobsearch_data);
                unset($plugins_list['wp-jobsearch']);
                
                $plugins_list = $jobsearch_arr + $plugins_list;
            }
        ...
        private function _content_users_install() {
            ...
            if (!empty($candidates_list)) {
                ...
                foreach ($candidates_list as $candidate_item) {
                    ...
                    $user_arr = array();
                    $user_arr['user_login'] = 'cand-dummy' . $candidate_count;
            ...
            if (!empty($employers_list)) {
                ...
                foreach ($employers_list as $employer_item) {
                    ...
                    $user_arr = array();
                    $user_arr['user_login'] = 'emp-dummy' . $employer_count;
                    $user_arr['first_name'] = $first_name;
                    $user_arr['last_name'] = $last_name;
                    $user_arr['user_pass'] = md5('demo12345');
                    $user_arr['user_email'] = $email;
            ...
            if (function_exists('careerfy_demo_data_get_path')) {
                ...
                if (is_dir($wp_bakery_dir)) {
                    ...
                    if (!empty($template_files_list)) {
                        ...
                        if (!empty($theme_mods)) {
                            $new_templates_arr = $templte_names_arr = array();
                            foreach ($theme_mods as $filcont_key => $filcont_val) {
                                if (isset($filcont_val['name']) && !in_array($filcont_val['name'], $templte_names_arr)) {
                                    $new_templates_arr[$filcont_key] = $filcont_val;
                                    $templte_names_arr[] = $filcont_val['name'];
                                }
                            }
                        }

                        $update_templates = update_option('wpb_js_templates', $new_templates_arr);
                    }
                }
            }
            return true;
