...
if (!class_exists('jobsearch_fjobs_package_expire_template')) {

    class jobsearch_fjobs_package_expire_template {
        ...
        public function __construct() {
            ...
            add_action('init', array($this, 'jobsearch_fjobs_package_expire_template_init'), 1, 0);
            add_filter('jobsearch_fjobs_package_expire_filter', array($this, 'jobsearch_fjobs_package_expire_filter_callback'), 1, 4);
            add_filter('jobsearch_email_template_settings', array($this, 'template_settings_callback'), 12, 1);
            add_action('jobsearch_fjobs_package_expire_email', array($this, 'jobsearch_fjobs_package_expire_callback'), 10, 2);
        }

        public function jobsearch_fjobs_package_expire_template_init() {
            $this->user = array();
            $this->order_id = '';
            
            $this->rand = rand(0, 99999);
            $this->group = 'job';
            $this->type = 'fjobs_package_expire';
            $this->filter = 'fjobs_package_expire';
            $this->email_template_db_id = 'fjobs_package_expire';
            $this->switch_label = esc_html__('Featured Jobs Package Expiry', 'wp-jobsearch');
            $this->default_subject = esc_html__('Featured Jobs Package Expire', 'wp-jobsearch');
            $this->default_recipients = '';
            $default_content = esc_html__('Default content', 'wp-jobsearch');
            $default_content = apply_filters('jobsearch_fjobs_package_expire_filter', $default_content, 'html', 'fjobs-package-expire', 'email-templates');
            $this->default_content = $default_content;
            $this->email_template_prefix = 'fjobs_package_expire';
            ...
            $this->codes = array(
                ...
                array(
                    'var' => '{total_jobs}',
                    'display_text' => esc_html__('Total Jobs', 'wp-jobsearch'),
                    'function_callback' => array($this, 'get_total_jobs'),
                ),
                array(
                    'var' => '{used_jobs}',
                    'display_text' => esc_html__('Used Jobs', 'wp-jobsearch'),
                    'function_callback' => array($this, 'get_used_jobs'),
                ),
                array(
                    'var' => '{remaining_jobs}',
                    'display_text' => esc_html__('Remaining Jobs', 'wp-jobsearch'),
                    'function_callback' => array($this, 'get_remaining_jobs'),
                ),
                array(
                    'var' => '{expiry_date}',
        ...
        public function jobsearch_fjobs_package_expire_callback($user, $order_id = '') {
            ...
            if (isset($template['switch']) && $template['switch'] == 1) {
                ...
                jobsearch_fjobs_package_expire_template::$is_email_sent1 = $this->is_email_sent;
                return true;
            }
        }
        ...
        public function jobsearch_fjobs_package_expire_filter_callback($html, $slug = '', $name = '', $ext_template = '') {
            ob_start();
            $html = '';
            $template = '';
            if ($ext_template != '') {
        ...
        public function template_settings_callback($email_template_options) {
            ...
            $email_template_options['fjobs_package_expire']['rand'] = $this->rand;
            $email_template_options['fjobs_package_expire']['email_template_prefix'] = $this->email_template_prefix;
            $email_template_options['fjobs_package_expire']['email_template_group'] = $this->email_template_group;
            $email_template_options['fjobs_package_expire']['default_var'] = $this->default_var;
        ...
        public function get_total_jobs() {
            ...
            $total_jobs = get_post_meta($trans_order_id, 'num_of_fjobs', true);
        ...
        public function get_used_jobs() {

            $trans_order_id = $this->order_id;
            
            $jobs_list = get_post_meta($trans_order_id, 'jobsearch_order_featc_list', true);

            if (!empty($jobs_list)) {
                $jobs_list_count = count(explode(',', $jobs_list));
            }
            
            $jobs_list_count = $jobs_list_count > 0 ? $jobs_list_count : 0;
        ...
        public function get_remaining_jobs() {
            ...
            $total_jobs = get_post_meta($trans_order_id, 'num_of_fjobs', true);
            $used_jobs = jobsearch_pckg_order_used_fjobs($trans_order_id);
    ...
    new jobsearch_fjobs_package_expire_template();
}