...
class Careerup_Socials_Widget extends Apus_Widget {
    public function __construct() {
        parent::__construct(
            'apus_socials_widget',
            esc_html__('Apus Socials', 'careerup'),
            array( 'description' => esc_html__( 'Socials for website.', 'careerup' ), )
        );
    ...
    public function scripts() {
        wp_enqueue_script( 'careerup-admin', get_template_directory_uri().'/js/admin.js', array( 'jquery' ), '1.0', true );
    }
    ...
    <div class="apus_socials">
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id( 'socials' )); ?>"><?php esc_html_e('Select socials:', 'careerup'); ?></label>
            <br>
        <?php
...
if ( function_exists('apus_framework_reg_widget') ) {
    apus_framework_reg_widget('Careerup_Socials_Widget');
}