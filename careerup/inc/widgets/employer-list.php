...
class Careerup_Widget_Employer_List extends Apus_Widget {
    public function __construct() {
        parent::__construct(
            'apus_widget_employer_list',
            esc_html__('Apus Simple Employers List', 'careerup'),
            array( 'description' => esc_html__( 'Show list of employer', 'careerup' ), )
        );
        $this->widgetName = 'employer_list';
    }

    public function getTemplate() {
        $this->template = 'employer-list.php';
    }
    ...
    public function form( $instance ) {
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('get_employers_by')); ?>">
                <?php echo esc_html__('Get employers by:', 'careerup' ); ?>
            </label>
            <br>
            <select id="<?php echo esc_attr($this->get_field_id('get_employers_by')); ?>" name="<?php echo esc_attr($this->get_field_name('get_employers_by')); ?>">
                <?php foreach ($get_employers_bys as $key => $title) { ?>
                    <option value="<?php echo esc_attr( $key ); ?>" <?php selected($instance['get_employers_by'], $key); ?> ><?php echo esc_html( $title ); ?></option>
                <?php } ?>
            </select>
        </p>
        <p>
...
<?php
    ...
    public function update( $new_instance, $old_instance ) {
        ...
        $instance['get_employers_by'] = ( ! empty( $new_instance['get_employers_by'] ) ) ? strip_tags( $new_instance['get_employers_by'] ) : '';
        return $instance;

    }
}
if ( function_exists('apus_framework_reg_widget') ) {
    apus_framework_reg_widget('Careerup_Widget_Employer_List');
}