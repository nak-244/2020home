...
if (!class_exists('jobsearch_cvs_package_expire_template')) {

    class jobsearch_cvs_package_expire_template {
        ...
        public function __construct() {
            ...
            add_action('init', array($this, 'jobsearch_cvs_package_expire_template_init'), 1, 0);
            add_filter('jobsearch_cvs_package_expire_filter', array($this, 'jobsearch_cvs_package_expire_filter_callback'), 1, 4);
            add_filter('jobsearch_email_template_settings', array($this, 'template_settings_callback'), 12, 1);
            add_action('jobsearch_cvs_package_expire_email', array($this, 'jobsearch_cvs_package_expire_callback'), 10, 2);
        }

        public function jobsearch_cvs_package_expire_template_init() {
            $this->user = array();
            $this->order_id = '';
            
            $this->rand = rand(0, 99999);
            $this->group = 'job';
            $this->type = 'cvs_package_expire';
            $this->filter = 'cvs_package_expire';
            $this->email_template_db_id = 'cvs_package_expire';
            $this->switch_label = esc_html__('CVs Package Expiry', 'wp-jobsearch');
            $this->default_subject = esc_html__('CVs Package Expire', 'wp-jobsearch');
            $this->default_recipients = '';
            $default_content = esc_html__('Default content', 'wp-jobsearch');
            $default_content = apply_filters('jobsearch_cvs_package_expire_filter', $default_content, 'html', 'cvs-package-expire', 'email-templates');
            $this->default_content = $default_content;
            $this->email_template_prefix = 'cvs_package_expire';
            ...
            $this->codes = array(
                ...
                array(
                    'var' => '{total_cvs}',
                    'display_text' => esc_html__('Total CVs', 'wp-jobsearch'),
                    'function_callback' => array($this, 'get_total_cvs'),
                ),
                array(
                    'var' => '{used_cvs}',
                    'display_text' => esc_html__('Used CVs', 'wp-jobsearch'),
                    'function_callback' => array($this, 'get_used_cvs'),
                ),
                array(
                    'var' => '{remaining_cvs}',
                    'display_text' => esc_html__('Remaining CVs', 'wp-jobsearch'),
                    'function_callback' => array($this, 'get_remaining_cvs'),
                ),
                array(
                    'var' => '{expiry_date}',
        ...
        public function jobsearch_cvs_package_expire_callback($user, $order_id = '') {
            ...
            if (isset($template['switch']) && $template['switch'] == 1) {
                ...
                jobsearch_cvs_package_expire_template::$is_email_sent1 = $this->is_email_sent;
                return true;
            }
        }
        ...
        public function jobsearch_cvs_package_expire_filter_callback($html, $slug = '', $name = '', $ext_template = '') {
            ob_start();
            $html = '';
            $template = '';
            if ($ext_template != '') {
        ...
        public function template_settings_callback($email_template_options) {
            ...
            $email_template_options['cvs_package_expire']['rand'] = $this->rand;
            $email_template_options['cvs_package_expire']['email_template_prefix'] = $this->email_template_prefix;
            $email_template_options['cvs_package_expire']['email_template_group'] = $this->email_template_group;
            $email_template_options['cvs_package_expire']['default_var'] = $this->default_var;
        ...
        public function get_total_cvs() {

            $trans_order_id = $this->order_id;
            
            $total_cvs = get_post_meta($trans_order_id, 'num_of_cvs', true);
        ...
        public function get_used_cvs() {

            $trans_order_id = $this->order_id;
            
            $cvs_list = get_post_meta($trans_order_id, 'jobsearch_order_cvs_list', true);

            if (!empty($cvs_list)) {
                $cvs_list_count = count(explode(',', $cvs_list));
            }
            
            $cvs_list_count = $cvs_list_count > 0 ? $cvs_list_count : 0;
            
            return $cvs_list_count;
        }
        
        public function get_remaining_cvs() {

            $trans_order_id = $this->order_id;
            
            $total_cvs = get_post_meta($trans_order_id, 'num_of_cvs', true);
            $used_cvs = jobsearch_pckg_order_used_cvs($trans_order_id);
    ...
    new jobsearch_cvs_package_expire_template();
}