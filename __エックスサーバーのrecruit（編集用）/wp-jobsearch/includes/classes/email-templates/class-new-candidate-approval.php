 ...
 * do_action('jobsearch_new_candidate_approval', $user_data, $post_id);
 * 
 */
if (!class_exists('jobsearch_new_candidate_approval_template')) {

    class jobsearch_new_candidate_approval_template {
        ...
        public function __construct() {
            ...
            add_action('init', array($this, 'jobsearch_new_candidate_approval_template_init'), 1, 0);
            add_filter('jobsearch_new_candidate_approval_filter', array($this, 'jobsearch_new_candidate_approval_filter_callback'), 1, 4);
            add_filter('jobsearch_email_template_settings', array($this, 'template_settings_callback'), 12, 1);
            add_action('jobsearch_new_candidate_approval', array($this, 'jobsearch_new_candidate_approval_callback'), 10, 2);
        }

        public function jobsearch_new_candidate_approval_template_init() {
            $this->user = array();
            $this->user_password = '';
            $this->rand = rand(0, 99999);
            $this->group = 'job';
            $this->type = 'new_candidate_approval';
            $this->filter = 'new_candidate_approval';
            $this->email_template_db_id = 'new_candidate_approval';
            $this->switch_label = esc_html__('To New Candidate Approval', 'wp-jobsearch');
            ...
            $default_content = apply_filters('jobsearch_new_candidate_approval_filter', $default_content, 'html', 'new-candidate-approval', 'email-templates');
            $this->default_content = $default_content;
            $this->email_template_prefix = 'new_candidate_approval';
        ...
        public function jobsearch_new_candidate_approval_callback($user = '', $password = '') {
            global $sitepress;
            $lang_code = '';
            ...
            if (isset($template['switch']) && $template['switch'] == 1) {
                ...
                jobsearch_new_candidate_approval_template::$is_email_sent1 = $this->is_email_sent;
            }
        }
        ...
        public function jobsearch_new_candidate_approval_filter_callback($html, $slug = '', $name = '', $ext_template = '') {
            ob_start();
            $html = '';
            $template = '';
            if ($ext_template != '') {
        ...
        public function template_settings_callback($email_template_options) {
            ...
            $email_template_options['new_candidate_approval']['rand'] = $this->rand;
            $email_template_options['new_candidate_approval']['email_template_prefix'] = $this->email_template_prefix;
            $email_template_options['new_candidate_approval']['email_template_group'] = $this->email_template_group;
            $email_template_options['new_candidate_approval']['default_var'] = $this->default_var;
    ...
    new jobsearch_new_candidate_approval_template();
}